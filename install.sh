...
curl -L -H "Cache-Control: no-cache" -o '/etc/systemd/system/tls-shunt-proxy.service' 'https://raw.githubusercontent.com/brendelvalim/config/master/tls-shunt-proxy.service'

if [ ! -f "/etc/tls-shunt-proxy/config.yaml" ]; then
  mkdir -p '/etc/tls-shunt-proxy'
  curl -L -H "Cache-Control: no-cache" -o '/etc/tls-shunt-proxy/config.yaml' 'https://raw.githubusercontent.com/brendelvalim/config/master/config.simple.yaml'
fi
